...
#Second Object
obj2 = ObjLoader("objects/cat.obj")
...
n_vertices2 = len(obj2.vertices) // (size_position + size_texture + size_normal)# number of vertices

scale = 2/dia2         # scale factor for the model
...
translation_mat2 = pyrr.matrix44.create_from_translation(-obj2.center)
scale_mat = pyrr.matrix44.create_from_scale([scale, scale, scale])

model_matrix = pyrr.matrix44.create_identity()
model_matrix2 = pyrr.matrix44.create_identity()
...
transformed_eye2 = eye-obj2.center
...
# *****************************************************************
catVao = glGenVertexArrays(1)
glBindVertexArray(catVao)
catVbo = glGenBuffers(1)
...
model_mat_loc2 = glGetUniformLocation(shader,"model_matrix2")
glUniformMatrix4fv(model_mat_loc2, 1, GL_FALSE, model_matrix2)
view_mat_loc2 = glGetUniformLocation(shader, "view_matrix")
projection_mat_loc2 = glGetUniformLocation(shader, "projection_matrix")
glUniformMatrix4fv(projection_mat_loc2, 1, GL_FALSE, projection_mat)

# **************************************************

gui = SimpleGUI("Two Viewports, Cat vs Wolf")
fov_slider = gui.add_slider("Field of View", min_value= 25, max_value= 125, initial_value=45)
...
while draw:
    ...
    scale_mat = pyrr.matrix44.create_from_scale([scale,scale,scale])
    translation_mat = translation_mat2
    rotated_eye = pyrr.matrix44.apply_to_vector(rot_max, transformed_eye)
    rotated_eye2 = pyrr.matrix44.apply_to_vector(rot_max, transformed_eye2)
    ...
    view_mat2 = pyrr.matrix44.create_look_at(rotated_eye2,target,up)
    projection_matrix2 = pyrr.matrix44.create_perspective_projection_matrix(fov_slider.get_value(),aspect,near,far)
    ...
    glUniformMatrix4fv(projection_mat_loc, 1, GL_FALSE, projection_matrix)
    glUniformMatrix4fv(model_mat_loc, 1, GL_FALSE, model_matrix)
    glBindVertexArray(catVao)              # Bind the VAO. That is, make it the active one.
    glDrawArrays(GL_TRIANGLES,
                 0,
                 n_vertices2)      # Draw the triangle
...
# Cleanup
glDeleteVertexArrays(1, [vao,catVao])
glDeleteBuffers(1, [vbo,catVbo])
